...
class DailyToyboxAdmin(commands.Cog):
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        # The buttons will still trigger this generic event, but discord.py's View system
        # requires the View to be added via bot.add_view() to handle callbacks persistently.
        # Since we use custom_ids, we can intercept them here if they aren't registered:
        if interaction.type == discord.InteractionType.component and interaction.data.get('component_type') == 2:
            custom_id = interaction.data.get('custom_id', '')
            if custom_id.startswith('admin_approve_') or custom_id.startswith('admin_reject_'):
                # We can handle it here or ensure `bot.add_view()` is called on startup.
                pass # Already handled if View is registered, or we can handle manually.
